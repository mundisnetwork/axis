...
pub fn write_transaction<W: io::Write>(
    ...
    for (instruction_index, instruction) in message.instructions.iter().enumerate() {
        ...
        let mut raw = true;
        if program_pubkey == mundis_vote_program::id() {
            if let Ok(vote_instruction) = limited_deserialize::<
                mundis_vote_program::vote_instruction::VoteInstruction,
            >(&instruction.data)
            {
        ...
        } else if program_pubkey == mundis_sdk::system_program::id() {
            if let Ok(system_instruction) = limited_deserialize::<
                mundis_sdk::system_instruction::SystemInstruction,
            >(&instruction.data)
            {
...
#[cfg(test)]
mod test {
    use {super::*, mundis_sdk::pubkey::Pubkey};
