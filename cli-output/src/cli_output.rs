...
use {
    ...
    mundis_account_decoder::parse_token::UiTokenAccount,
    mundis_clap_utils::keypair::SignOnly,
    mundis_client::rpc_response::{
            ...
            match &keyed_reward.reward {
                Some(reward) => {
                    writeln!(
                        ...
                        lamports_to_mdis(reward.amount),
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for account in &self.accounts {
            writeln!(
                ...
                "{:<44}  {}",
                account.address,
                &format!("{} MUNDIS", lamports_to_mdis(account.lamports))
            )?;
        }
        Ok(())
    }
...
impl fmt::Display for CliSupply {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln_name_value(f, "Total:", &format!("{} MUNDIS", lamports_to_mdis(self.total)))?;
        writeln_name_value(
            f,
            "Circulating:",
            &format!("{} MUNDIS", lamports_to_mdis(self.circulating)),
        )?;
        writeln_name_value(
            f,
            "Non-Circulating:",
            &format!("{} MUNDIS", lamports_to_mdis(self.non_circulating)),
        )?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !self.encoded_confirmed_block.rewards.is_empty() {
            ...
            writeln!(
                ...
                "Total Rewards: {}◎{:<12.9}",
                sign,
                lamports_to_mdis(total_rewards.abs() as u64)
            )?;
        }
