use {
    ...
    log::*,
    rand::{thread_rng, Rng},
    mundis_address_lookup_table_program::state::AddressLookupTable,
    mundis_sdk::{
...
impl Accounts {
    ...
    pub fn load_lookup_table_addresses(
        ...
        if table_account.owner() == &mundis_address_lookup_table_program::id() {
        ...
        if tx.signatures().is_empty() && fee != 0 {
            ...
            for (i, key) in message.account_keys_iter().enumerate() {
                let account = if !message.is_non_loader_key(i) {
                    ...
                    if mundis_sdk::sysvar::instructions::check_id(key) {
...
mod tests {
    use {
        ...
        mundis_address_lookup_table_program::state::LookupTableMeta,
        mundis_program_runtime::invoke_context::Executors,
        mundis_sdk::{
    ...
    fn test_load_accounts_invalid_account_for_fee() {
        ...
        let account = AccountSharedData::new(1, 1, &mundis_sdk::pubkey::new_rand()); // <-- owner is not the system program
        accounts.push((key0, account));
    ...
    fn test_load_lookup_table_addresses_invalid_account_data() {
        ...
        let invalid_table_account =
            AccountSharedData::new(1, 0, &mundis_address_lookup_table_program::id());
        accounts.store_slow_uncached(0, &invalid_table_key, &invalid_table_account);
    ...
    fn test_instructions() {
        ...
        let instructions_key = mundis_sdk::sysvar::instructions::id();
        let keypair = Keypair::new();
        let instructions = vec![CompiledInstruction::new(1, &(), vec![0, 1])];
        let tx = Transaction::new_with_compiled_instructions(
            &[&keypair],
            &[mundis_sdk::pubkey::new_rand(), instructions_key],
            Hash::default(),
            vec![native_loader::id()],
            instructions,
        );
