...
use {
    ...
    mundis_ledger::{blockstore::Blockstore, blockstore_db::AccessType},
    mundis_sdk::{clock::Slot, pubkey::Pubkey, signature::Signature},
...
async fn upload(
    ...
    let bigtable = mundis_storage_bigtable::LedgerStorage::new(false, None, None)
        .await
        .map_err(|err| format!("Failed to connect to storage: {:?}", err))?;

    mundis_ledger::bigtable_upload::upload_confirmed_blocks(
        Arc::new(blockstore),
        bigtable,
        starting_slot,
        ending_slot,
...
async fn delete_slots(slots: Vec<Slot>, dry_run: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bigtable = mundis_storage_bigtable::LedgerStorage::new(read_only, None, None)
        .await
        .map_err(|err| format!("Failed to connect to storage: {:?}", err))?;

    mundis_ledger::bigtable_delete::delete_confirmed_blocks(bigtable, slots, dry_run).await
}

async fn first_available_block() -> Result<(), Box<dyn std::error::Error>> {
    let bigtable = mundis_storage_bigtable::LedgerStorage::new(true, None, None).await?;
...
async fn compare_blocks(
    ...
    let owned_bigtable = mundis_storage_bigtable::LedgerStorage::new(false, None, None)
        .await
    ...
    let reference_bigtable =
        mundis_storage_bigtable::LedgerStorage::new(false, None, Some(credential_path))
            .await
