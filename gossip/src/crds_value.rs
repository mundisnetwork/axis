...
impl SnapshotHashes {
    ...
    pub(crate) fn new_rand<R: Rng>(rng: &mut R, pubkey: Option<Pubkey>) -> Self {
        ...
        let hashes = std::iter::repeat_with(|| {
            ...
            let hash = mundis_sdk::hash::new_rand(rng);
            (slot, hash)
        })
        .take(num_hashes)
        .collect();
...
pub struct Version {
    pub from: Pubkey,
    pub wallclock: u64,
    pub version: mundis_version::Version,
}
...
impl Version {
    pub fn new(from: Pubkey) -> Self {
        Self {
            from,
            wallclock: timestamp(),
            version: mundis_version::Version::default(),
        }
    }
...
mod test {
    use {
        ...
        mundis_vote_program::{vote_instruction, vote_state},
