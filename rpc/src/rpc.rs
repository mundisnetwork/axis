//! The `rpc` module implements the Mudis RPC interface.
...
use {
    ...
    mundis_faucet::faucet::request_airdrop_transaction,
    ...
    mundis_perf::packet::PACKET_DATA_SIZE,
    mundis_runtime::{
    ...
    mundis_send_transaction_service::{
        send_transaction_service::{SendTransactionService, TransactionInfo},
        tpu_info::NullTpuInfo,
    },
    mundis_storage_bigtable::Error as StorageError,
    mundis_streamer::socket::SocketAddrSpace,
    mundis_transaction_status::{
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<mundis_storage_bigtable::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, mundis_storage_bigtable::Error>,
    ) -> Result<()> {
        if let Err(mundis_storage_bigtable::Error::BlockNotFound(slot)) = result {
    ...
    pub fn get_stake_activation(
        ...
        if bank.epoch().saturating_sub(epoch) > mundis_sdk::stake_history::MAX_ENTRIES as u64 {
            return Err(Error::invalid_params(format!(
        ...
        let stake_history =
            mundis_sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
...
fn get_anima_token_mint_filter(program_id: &Pubkey, filters: &[RpcFilterType]) -> Option<Pubkey> {
    ...
    if data_size_filter == Some(TokenAccount::packed_len() as u64) {
        ...
        debug!("anima_token program filters do not match by-mint index requisites");
        None
    }
}
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `mundis-validator wait-for-restart-window` to not require this method, so
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_version(&self, _: Self::Metadata) -> Result<RpcVersionInfo> {
            ...
            let version = mundis_version::Version::default();
            Ok(RpcVersionInfo {
                mundis_core: version.to_string(),
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        mundis_ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == mundis_sdk::slot_history::Check::Found {
...
pub fn create_test_transactions_and_populate_blockstore(
    ...
    let success_tx = mundis_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair1.pubkey(),
        rent_exempt_amount,
        blockhash,
    ...
    let entry_1 = mundis_entry::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx = mundis_sdk::system_transaction::transfer(
        keypair2,
        &keypair3.pubkey(),
    ...
    let entry_2 = mundis_entry::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
    let entries = vec![entry_1, entry_2];

    let shreds = mundis_ledger::blockstore::entries_to_test_shreds(
        entries.clone(),
        slot,
        previous_slot,
        true,
    ...
    assert_eq!(
        mundis_ledger::blockstore_processor::process_entries_for_tests(
            &bank,
            entries,
            true,
            Some(
...
pub mod tests {
    use {
        ...
        mundis_client::rpc_filter::{Memcmp, MemcmpEncodedBytes},
        mundis_gossip::{contact_info::ContactInfo, socketaddr},
        mundis_ledger::{
    ...
    fn test_rpc_get_leader_schedule() {
        ...
        .iter()
        {
            ...
            assert_eq!(
                bob_schedule.len(),
                mundis_ledger::leader_schedule_utils::leader_schedule(bank.epoch(), &bank)
                    .unwrap()
                    .get_slot_leaders()
                    .len()
            );
    ...
    fn test_rpc_get_account_info() {
        ...
        let address = mundis_sdk::pubkey::new_rand();
    ...
    fn test_rpc_get_program_accounts() {
        ...
        let new_program_id = mundis_sdk::pubkey::new_rand();
        ...
        let authority = mundis_sdk::pubkey::new_rand();
        let instruction = system_instruction::create_nonce_account(
            &alice.pubkey(),
    ...
    fn test_token_rpcs() {
        ...
        let other_token_account_pubkey = mundis_sdk::pubkey::new_rand();
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        let expect58 = Error::invalid_params(format!(
            "encoded mundis_sdk::transaction::Transaction too large: {} bytes (max: encoded/raw {}/{})",
        ...
        let expect = Error::invalid_params(format!(
            "encoded mundis_sdk::transaction::Transaction too large: {} bytes (max: {} bytes)",
