//! The `rpc_service` module implements the Mundis JSON RPC service.
...
use {
    ...
    regex::Regex,
    mundis_client::rpc_cache::LargestAccountsCache,
    ...
    mundis_perf::thread::renice_this_thread,
    mundis_poh::poh_recorder::PohRecorder,
    mundis_runtime::{
    ...
    match path {
        "/v0/circulating-supply" => {
            ...
            let non_circulating_supply =
                mundis_runtime::non_circulating_supply::calculate_non_circulating_supply(&bank)
    ...
    pub fn new(
        ...
        let (bigtable_ledger_storage, _bigtable_ledger_upload_service) =
            if config.enable_bigtable_ledger_storage || config.enable_bigtable_ledger_upload {
                runtime
                    .block_on(mundis_storage_bigtable::LedgerStorage::new(
...
mod tests {
    ...
    fn test_rpc_new() {
        ...
        let rpc_addr = SocketAddr::new(
            ip_addr,
            mundis_net_utils::find_available_port_in_range(ip_addr, (10000, 65535)).unwrap(),
        );
        ...
        assert_eq!(thread.name().unwrap(), "mundis-jsonrpc");
