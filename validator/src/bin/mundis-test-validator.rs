...
use {
    ...
    mundis_core::tower_storage::FileTowerStorage,
    mundis_faucet::faucet::{run_local_faucet_with_port, FAUCET_PORT},
 ...
 * 40MB-150MB range when running `mundis-test-validator`
 */
...
fn main() {
    ...
    let matches = App::new("mundis-test-validator")
        ...
        .arg({
            ...
            if let Some(ref config_file) = *mundis_cli_config::CONFIG_FILE {
        ...
        .arg(
            Arg::with_name("json_rpc_url")
                ...
                .help(
                    "URL for the Mundis JSON RPC or moniker (or their first letter): \
        ...
        .arg(
            Arg::with_name("account")
                ...
                .multiple(true)
                .help(
                    "Load an account from the provided JSON file (see `mundis account --help` on how to dump \
    ...
    mundis_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    let cli_config = if let Some(config_file) = matches.value_of("config_file") {
        mundis_cli_config::Config::load(config_file).unwrap_or_default()
    } else {
        mundis_cli_config::Config::default()
    };
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        mundis_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        mundis_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
