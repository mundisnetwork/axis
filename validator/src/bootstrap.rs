...
fn verify_reachable_ports(
    ...
    mundis_net_utils::verify_reachable_ports(
...
fn check_vote_account(
    ...
    let vote_state = mundis_vote_program::vote_state::VoteState::from(&vote_account);
...
mod without_incremental_snapshots {
    ...
    pub fn rpc_bootstrap(
        ...
        loop {
            ...
            Ok(rpc_version) => {
                info!("RPC node version: {}", rpc_version.mundis_core);
                Ok(())
            }
        ...
        .map(|_| {
            if let Some(url) = bootstrap_config.check_vote_account.as_ref() {
                ...
                .unwrap_or_else(|err| {
                    ...
                    // incorrect `mundis-validator` command-line arguments) rather than the
                    // RPC node failing.
                    //
