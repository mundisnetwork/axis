use {
    ...
    log::*,
    mundis_clap_utils::input_parsers::{lamports_of_sol, value_of},
    mundis_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    mundis_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = mundis_cli_config::Config::default().keypair_path;

    mundis_logger::setup_with_default("mundis=info");
    mundis_metrics::set_panic_hook("faucet", /*version:*/ None);
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in MUNDIS"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in MUNDIS"),
        )
        .arg(
