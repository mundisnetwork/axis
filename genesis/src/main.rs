...
use {
    ...
    mundis_entry::poh::compute_hashes_per_tick,
    mundis_genesis::{genesis_accounts::add_genesis_accounts, Base64Account},
    ...
    mundis_vote_program::vote_state::{self, VoteState},
    std::{
        collections::HashMap,
        error,
        fs::File,
...
fn main() -> Result<(), Box<dyn error::Error>> {
    let default_faucet_pubkey = mundis_cli_config::Config::default().keypair_path;
    ...
    mundis_stake_program::add_genesis_accounts(&mut genesis_config);
    if genesis_config.cluster_type == ClusterType::Development {
        mundis_runtime::genesis_utils::activate_all_features(&mut genesis_config);
    }
...
mod tests {
    use {
        super::*,
        mundis_sdk::genesis_config::GenesisConfig,
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            mundis_sdk::pubkey::new_rand().to_string(),
            Base64Account {
                owner: mundis_sdk::pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,
