...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend(&[".config", "mundis", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The Mundis CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq)]
pub struct Config {
    /// The RPC address of a Mundis validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// Mundis documentation][rpcdocs].
    ///
    ...
    /// documentation for `mundis_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `mundis_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from Mundis addresses to human-readable names.
    ///
    ...
    /// `mundis_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend(&[".config", "mundis", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet.mundis.io".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.mundis.io"),
            "ws://api.devnet.mundis.io/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.mundis.io"),
            "wss://api.devnet.mundis.io/".to_string()
        );
