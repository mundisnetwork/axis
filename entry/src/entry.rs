...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = mundis_perf::perf_libs::locate_perf_libs()
            {
                mundis_perf::perf_libs::append_to_ld_library_path(
...
impl EntrySlice for [Entry] {
    ...
    fn verify_cpu_x86_simd(&self, start_hash: &Hash, simd_len: usize) -> EntryVerificationState {
        use mundis_sdk::hash::HASH_BYTES;
        let now = Instant::now();
        let genesis = [Entry {
            num_hashes: 0,
            hash: *start_hash,
...
mod tests {
    use {
        super::*,
        mundis_perf::test_tx::{test_invalid_tx, test_tx},
        mundis_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
