...
pub fn normalize_to_url_if_moniker<T: AsRef<str>>(url_or_moniker: T) -> String {
    match url_or_moniker.as_ref() {
        "m" | "mainnet-beta" => "https://api.mainnet.mundis.io",
        "t" | "testnet" => "https://api.testnet.mundis.io",
        "d" | "devnet" => "https://api.devnet.mundis.io",
    ...
    T: AsRef<str> + Display,
{
    ...
    if mundis_perf::thread::is_renice_allowed(adjustment) {
        Ok(())
    } else {
        Err(String::from(
