//! The mundis-program-test crate provides a BanksClient-based test framework BPF programs
...
pub fn builtin_process_instruction(
    process_instruction: mundis_sdk::entrypoint::ProcessInstruction,
...
/// Converts a `mundis-program`-style entrypoint into the runtime's entrypoint style, for
    ...
    ($process_instruction:expr) => {
        Some(
            |first_instruction_account: usize,
             input: &[u8],
             invoke_context: &mut mundis_program_test::InvokeContext| {
...
struct SyscallStubs {}
impl mundis_sdk::program_stubs::SyscallStubs for SyscallStubs {
...
impl ProgramTest {
    ...
    fn setup_bank(
    ...
    ) {
        {
            ...
            ONCE.call_once(|| {
                mundis_sdk::program_stubs::set_syscall_stubs(Box::new(SyscallStubs {}));
            });
        }
        ...
        if self.use_bpf_jit {
            add_builtin!(mundis_bpf_loader_program_with_jit!());
            add_builtin!(mundis_bpf_loader_upgradeable_program_with_jit!());
        } else {
            add_builtin!(mundis_bpf_loader_program!());
