...
impl From<VaultError> for InstructionError {
...
impl PrintInstructionError for VaultError {
    fn print<E>(&self)
        where
...
#[cfg(test)]
mod tests {
    use crate::error::{PrintInstructionError, VaultError};

    fn generate_error() -> Result<(), VaultError> {
        Err(VaultError::AuthorityDoesNotMatch)
    }
