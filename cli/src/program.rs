use {
    ...
    log::*,
    mundis_account_decoder::{UiAccountEncoding, UiDataSliceConfig},
    mundis_bpf_loader_program::{syscalls::register_syscalls, BpfError, ThisInstructionMeter},
                    ...
                    SubCommand::with_name("deploy")
                        ...
                        .arg(
                            Arg::with_name("allow_excessive_balance")
                                ...
                                .takes_value(false)
                                .help("Use the designated program id even if the account already holds a large balance of MUN")
                        ),
                )
                .subcommand(
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!(
        "`mundis-keygen recover` and the following {}-word seed phrase:",
        words
    );
    eprintln!("{}\n{}\n{}", divider, phrase, divider);
    eprintln!("To resume a deploy, pass the recovered keypair as the");
    eprintln!("[BUFFER_SIGNER] to `mundis write-buffer'.");
    eprintln!("Or to recover the account's lamports, pass it as the");
    eprintln!(
        "[BUFFER_ACCOUNT_ADDRESS] argument to `mundis program close`.\n{}",
        divider
    );
}
...
mod tests {
    use {
        ...
        serde_json::Value,
        mundis_cli_output::OutputFormat,
        mundis_sdk::signature::write_keypair_file,
    };
