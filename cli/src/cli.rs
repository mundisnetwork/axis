use {
    ...
    mundis_clap_utils::{self, input_parsers::*, input_validators::*, keypair::*},
    mundis_cli_output::{
    ...
    mundis_vote_program::vote_state::VoteAuthorize,
...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} MUNDIS)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} MUNDIS)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} MUNDIS) + fee ({1} MUNDIS)")]
...
impl CliConfig<'_> {
    fn default_keypair_path() -> String {
        mundis_cli_config::Config::default().keypair_path
    }

    fn default_json_rpc_url() -> String {
        mundis_cli_config::Config::default().json_rpc_url
    }

    fn default_websocket_url() -> String {
        mundis_cli_config::Config::default().websocket_url
    }
    ...
    pub fn compute_websocket_url_setting(
        ...
        Self::first_nonempty_setting(vec![
            ...
            (
                ...
                mundis_cli_config::Config::compute_websocket_url(&normalize_to_url_if_moniker(
                    json_rpc_cmd_url,
                )),
            ),
            (
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            get_clap_app(
                crate_name!(),
                crate_description!(),
                mundis_version::version!(),
            )
            .gen_completions_to("mundis", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of mundis-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        // Request an airdrop from Mundis Faucet;
        ...
        CliCommand::TransferToken {
            ...
            ref multisigner_pubkeys,
            ref tx_info,
        } => process_transfer_token_command(
            &rpc_client,
            config,
            *token,
            *ui_amount,
        ...
        CliCommand::WrapToken {
            ...
            wrapped_sol_account,
            ref tx_info
        } => process_wrap_token_command(&rpc_client, config, *mdis, *wallet_address, *wrapped_sol_account, tx_info),
...
mod tests {
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(mundis_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_parse_command() {
        ...
        let from_pubkey = Some(mundis_sdk::pubkey::new_rand());
        let from_str = from_pubkey.unwrap().to_string();
        for (name, program_id) in &[
            ("STAKE", stake::program::id()),
            ("VOTE", mundis_vote_program::id()),
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "0.00000005 MUNDIS");
        ...
        let new_authorized_pubkey = mundis_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = mundis_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = mundis_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = mundis_sdk::pubkey::new_rand();
    ...
    fn test_parse_transfer_subcommand() {
        ...
        //Test Transfer Subcommand, MUNDIS
