use {
    ...
    mundis_transaction_status::{EncodedTransaction, UiTransactionEncoding},
    ...
    fn wallet_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("airdrop")
                .about("Request MUNDIS from a faucet")
                .arg(
                    Arg::with_name("amount")
                        ...
                        .validator(is_amount)
                        .required(true)
                        .help("The airdrop amount to request, in MUNDIS"),
                )
                .arg(
            ...
            SubCommand::with_name("transfer")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .required(true)
                        .help("The amount to send, in MUNDIS; accepts keyword ALL"),
                )
                .arg(
                    pubkey!(Arg::with_name("from")
                        .long("from")
...
fn resolve_derived_address_program_id(matches: &ArgMatches<'_>, arg_name: &str) -> Option<Pubkey> {
    matches.value_of(arg_name).and_then(|v| match v {
        ...
        "VOTE" => Some(mundis_vote_program::id()),
...
pub fn process_airdrop(
    ...
    if let Ok(signature) = result {
        ...
        if current_balance < pre_balance.saturating_add(lamports) {
            ...
            println!("Run `mundis confirm -v {:?}` for more info", signature);
            Ok("".to_string())
        } else {
