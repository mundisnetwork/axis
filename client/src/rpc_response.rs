use {
    crate::client_error,
    mundis_account_decoder::{parse_token::UiTokenAmount, UiAccount},
    mundis_sdk::{
...
pub struct RpcVersionInfo {
    /// The current version of mundis-core
    pub mundis_core: String,
    /// first 4 bytes of the FeatureSet identifier
    pub feature_set: Option<u32>,
}
...
impl fmt::Debug for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.mundis_core)
    }
}

impl fmt::Display for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Some(version) = self.mundis_core.split_whitespace().next() {
