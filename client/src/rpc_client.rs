//! Communication with an Axis node over RPC.
//!
//! Software that interacts with the Mundis blockchain, whether querying its
//! state or submitting transactions, communicates with an Axis node over
...
/// A client of a remote Axis node.
///
/// `RpcClient` communicates with a Axis node over [JSON-RPC], with the
/// [Mundis JSON-RPC protocol][jsonprot]. It is the primary Rust interface for
...
///
/// ```
/// # use mundis_sdk::system_transaction;
/// # use mundis_client::rpc_client::RpcClient;
/// # use mundis_client::client_error::ClientError;
...
impl RpcClient {
    ...
    fn get_node_version(&self) -> Result<semver::Version, RpcError> {
        ...
        if let Some(version) = &*r_node_version {
            ...
            let node_version = semver::Version::parse(&node_version.mundis_core).map_err(|e| {
    ...
    ///
    /// [`TransactionError`]: mundis_sdk::transaction::TransactionError
    ///
    /// This function only searches a node's recent history, including all
    /// recent slots, plus up to
    /// [`MAX_RECENT_BLOCKHASHES`][mundis_sdk::clock::MAX_RECENT_BLOCKHASHES]
    ...
    /// #     client_error::ClientError,
    /// # };
    /// # use mundis_sdk::slot_history::Slot;
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let start_slot = 1;
    /// let limit = 3;
    ...
    ///
    /// ```
    /// # use mundis_transaction_status::UiTransactionEncoding;
    /// # use mundis_client::{
    /// #     rpc_client::RpcClient,
    /// #     client_error::ClientError,
    /// # };
    ...
    /// #     commitment_config::CommitmentConfig,
    /// # };
    /// # use mundis_account_decoder::UiAccountEncoding;
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let alice = Keypair::new();
    /// # let bob = Keypair::new();
    /// let pubkeys = vec![alice.pubkey(), bob.pubkey()];
    ...
    /// #     commitment_config::CommitmentConfig,
    /// # };
    /// # use mundis_account_decoder::{UiDataSliceConfig, UiAccountEncoding};
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let alice = Keypair::new();
    /// # let base64_bytes = "\
